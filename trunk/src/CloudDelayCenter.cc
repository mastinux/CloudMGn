...
void CloudDelayCenter::processReturningCloudAppJob(cMessage *msg){
    ...
    //TODO controllare che il valore del DelayTime sia diverso da 0
    ...
    if (!checkTimeoutExpired(check_and_cast<CloudAppJob *>(job))){
        send(job, "out");
    }
}


bool CloudDelayCenter::checkTimeoutExpired(CloudAppJob *job, bool autoremove){
